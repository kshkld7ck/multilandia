...
function MoviePage({ config }) {
    ...
    return <section className="movie-page">
        <div className="container">
            <h3>{config.title} <button className="btn btn_primary"><svg width="23" height="21" viewBox="0 0 23 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {Object.values(config.properties).map((el) => {
                        return <div className="movie-page__info-item">
                            ...
                            <div className="movie-page__info-value">
                                {el.items.map((item) => <Link to={item.url || "/"}>{item.name}</Link>)}
                            </div>
                        </div>
                    })}
                </div>
